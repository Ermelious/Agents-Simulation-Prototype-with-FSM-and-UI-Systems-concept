				...
				switch (Tools.current)
				{
					...
					case Tool.Scale:
						if (selectedAnchorPoint != -1)
						{
							...
							for (int i = -1; i <= 1; i += 2)
							{
								...
								{
									var fmh_246_50_638022334213650288 = Quaternion.identity; var nPos = Handles.FreeMoveHandle(hPos,
					...
					case Tool.Rect:
						...
						foreach (var p in points)
						{
							...
							{
								var fmh_298_87_638022334213691960 = Quaternion.identity; var newPos = Handles.FreeMoveHandle($"Handle-{p.index}".GetHashCode(), p.pos,
